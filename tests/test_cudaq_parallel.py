...
def test_resolve_cpu_max_parallel_instances_default() -> None:
    assert resolve_cpu_max_parallel_instances({}) == 1
    assert resolve_cpu_max_parallel_instances({"cpu_max_parallel_instances": 4}) == 4


def test_resolve_cpu_max_parallel_instances_clamps_minimum() -> None:
    assert resolve_cpu_max_parallel_instances({"cpu_max_parallel_instances": 0}) == 1
    assert resolve_cpu_max_parallel_instances({"cpu_max_parallel_instances": -2}) == 1


def test_resolve_cpu_max_parallel_instances_env_overrides_yaml(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv(CPU_PARALLEL_ENV, "6")
    assert resolve_cpu_max_parallel_instances({"cpu_max_parallel_instances": 2}) == 6
    monkeypatch.delenv(CPU_PARALLEL_ENV, raising=False)


def test_resolve_cpu_max_parallel_instances_env_empty_uses_yaml(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv(CPU_PARALLEL_ENV, "  ")
    assert resolve_cpu_max_parallel_instances({"cpu_max_parallel_instances": 3}) == 3
    monkeypatch.delenv(CPU_PARALLEL_ENV, raising=False)
...
def test_run_cirq_parallel_batch_two_instances_inline(
    ...
    serializable = {
        ...
        "cpu_max_parallel_instances_effective": 2,
    }
...
def test_run_brute_force_parallel_batch_two_instances_inline(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """brute_force path with inline executor so BruteForceSolver can be mocked in-process."""
    import experiments.cudaq_parallel as cqp
    ...
    specs = [
        CudaqParallelJobSpec(
            k=k,
            instance_json_path=str(inst_dir / f"instance_{k}.json"),
            status_label=f"n_cities=5 depth=None inst={k}",
            run_config=run_config,
            instance_config_dict={"n_cities": 5, "seed": 0},
            solver_config_serializable=serializable,
    ...
    with patch("solvers.brute_force.BruteForceSolver", return_value=mock_solver):
        batch = run_cudaq_parallel_batch(
            specs,
            max_workers=2,
            solutions_write_fn=_write,
...
def test_run_sa_parallel_batch_two_instances_inline(
    ...
    with patch("solvers.simulated_annealing.SimulatedAnnealingSolver", return_value=mock_solver):
        batch = run_cudaq_parallel_batch(
            specs,
            max_workers=2,
            solutions_write_fn=_write,
...
def test_run_experiment_cirq_parallel_path_not_used_when_workers_one(
    ...
    """With cpu_max_parallel_instances=1, ProcessPoolExecutor must not start (Cirq)."""
    import experiments.cudaq_parallel as cqp
    from experiments import main_experiment_workflow as mew
...
def test_run_experiment_brute_force_parallel_path_not_used_when_workers_one(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """With cpu_max_parallel_instances=1, ProcessPoolExecutor must not start (brute_force)."""
    import experiments.cudaq_parallel as cqp
    from experiments import main_experiment_workflow as mew
    ...
    with patch.object(mew.BruteForceSolver, "solve") as mock_solve:
        from solvers.base import SolverResult
...
def test_run_experiment_sa_parallel_path_not_used_when_workers_one(
    ...
    with patch.object(mew.SimulatedAnnealingSolver, "solve") as mock_solve:
        from solvers.base import SolverResult
